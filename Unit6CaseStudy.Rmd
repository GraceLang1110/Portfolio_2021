...
```{r random forest using azureml optimal parms with cv}
set.seed(123)
...
pred = predict(prune.bank, tree.data.test)
print(pred)
pred[(pred>0.5)] = 1
pred[(pred<0.5)] = 0
pred_y = as.factor(pred)
test_y = as.factor(tree.data.test$isSpam)
print(pred_y)
...
```{r xgboost using grid search with cv}
# parameter search
